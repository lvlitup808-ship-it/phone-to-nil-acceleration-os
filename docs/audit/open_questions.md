...
| 5 | `ground_contact_time_first_step = 0.45 × step-to-step ms`, reported as `insufficient_data` below a 90 ms floor (it used to be clamped to 90 and labelled `ok`) | Heuristic with no source; the 0.45 factor and 90 ms floor are unverified. Needs high-fps film or a contact mat | CV lead | GCT cue meaning |
...
| 19 | ~~Gate diversity fields missing from the manifest~~ | **Resolved:** manifest schema 1.1.0 has `athlete_id`, `surface`, `lighting` (intake-form vocabularies), required on any clip with film present and checked in CI. Values stay `null` until real clips are filmed | Film lead | — |
...
| 22 | ~~Docker image build~~ | **Resolved:** the `ci.yml` `docker` job passed on PR #12 (head `6d719c6`); the image was built, not run | Eng | — |
...
| 25 | `/pose/assess` now returns 404 for an unknown `consent_id` and 403 for a revoked one (it used to accept both and, for revoked, still write debug files) | Stricter Slice 2 behaviour; confirm no client relies on sending unregistered consent ids | Product | Merge of this PR |
| 26 | Consent revocation deletes the consent's clips plus the athlete's clips uploaded with no `consent_id`. Whether revocation should be keyed by athlete or by consent everywhere is not decided | Legal / product decision | Legal + product | — |
| 27 | Standalone valuation service (`services/valuation/app.py`) returns `status: schema_only` with nulls but does not report the golden-set gate like the main API | Numbers never leak; whether the service should know about the gate is a design call | Eng | — |
| 28 | `pose_error` on `/pose/assess` includes the adapter's exception text | Helps debugging; may expose internals to callers. Keep or reduce to the exception type? | Eng | — |
| 29 | Slice 1 units for `stride_frequency` (Hz) and `deceleration` (ms) are assumptions; no code computes those metrics, callers supply them | Needs a metric definition | Product + CV lead | — |
